...
$allowed_sorts = ['last_name', 'created_at']; // Только фамилия и дата добавления
...
// Получаем клиентов с учетом сортировки и поиска
$query = "SELECT * FROM customers $where ORDER BY $sort $order";
...
// Получаем автомобили
$carsStmt = $pdo->query("
    SELECT customer_id, GROUP_CONCAT(CONCAT(make, ' ', model) SEPARATOR ', ') as cars
    FROM customer_cars 
    GROUP BY customer_id
");
$carsData = $carsStmt->fetchAll(PDO::FETCH_KEY_PAIR);
?>
...
<div class="top-actions">
    <div class="add-btn-wrapper">
        <a href="add.php" class="btn-main">Добавить клиента</a>
    </div>
            ...
            <?php if (empty($customers)): ?>
                <tr>
                    <td colspan="6" class="no-results">
                        <?= empty($search) ? 'Нет клиентов в базе' : 'Ничего не найдено' ?>
                    </td>
                </tr>
            <?php else: ?>
                ...
                <tr>
                    ...
                    <td><?= isset($carsData[$customer['id']]) ? htmlspecialchars($carsData[$customer['id']]) : 'Нет авто' ?></td>
                    <td class="actions">
                        <a href="view.php?id=<?= $customer['id'] ?>" class="btn-action">Просмотр</a>
                        <a href="edit.php?id=<?= $customer['id'] ?>" class="btn-action">Редактировать</a>
                        <a href="delete.php?id=<?= $customer['id'] ?>" class="btn-action" onclick="return confirm('Вы уверены?')">Удалить</a>
                    </td>
                </tr>
                <?php endforeach; ?>
            <?php endif; ?>
